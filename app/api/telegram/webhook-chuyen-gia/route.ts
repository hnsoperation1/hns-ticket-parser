...
async function handleCallback(cq: TelegramCallbackQuery) {
  ...
  if (action === 'cancel') {
    ...
    await answerCallbackQuery(cq.id, '🚫 Đã huỷ.', token);
    if (cq.message) {
...
export async function POST(req: NextRequest) {
  ...
  if (!parseResult.success || parseResult.bookings.length === 0) {
    ...
    await sendMessage(msg.chat.id, '❌ Oh no! Em không đọc được thông tin booking. Chị kiểm tra lại định dạng tin nhắn hoặc bảo Chat GPT xử lý giúp em.', token);
    return NextResponse.json({ ok: true });
  }
